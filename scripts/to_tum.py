...
def parse_args():
    ...
    parser.add_argument(
        ...
        "--input",
        required=True,
        help="Full path to images.txt created by colmap containing features and poses of the registered cameras.",
    )
    # parser.add_argument("--output", required=True, help="Full file paht to output tum file (example: ~/$WORKSPACE/cam_trajectory.tum).")
    parser.add_argument(
        "--lower_bound",
        type=int,
        help="Ignore poses below this nsec integer timestamp. Useful in case you don't want to export the train images.",
    )
...
def main():
    ...
    with open(image_txt_file) as csv_file:
        ...
        i = 1
        df = pd.DataFrame(
            columns=("idx", "timestamp", "x", "y", "z", "q_x", "q_y", "q_z", "q_w")
        )
        for row in csv_reader:
            ...
            if line_count % 2 == 0:
                ...
                pose = transformation_matrix_from_quat_xyzw(
                    np.array(quat), np.array(pos)
                )
                pose_inv = inv(
                    pose
                )  # invert since original entries are world poses with respect to cameras
