 ...
 * @author kraxel
 * @brief Data container for tracking residuals of ceres factors during optimization. Can be used for debugging or evaluation purposes.
 * @version 0.1
 * @date 2025-03-05
 *
...
/**
 * @brief Structure that can track (stalk) the residual vector, calculated from a ceres cost-function during active optimization. Ptr of
 * stalker object needs to be attached to a residual-exposed cost-functor during functor creation AND to an extra data container (residuals
 * tracker) to view the obtained residuals. Residuals can only be obtained by stalker when being actively registered and supervied by an
 * FusionEvaluationObject.
 *
...
class ResidualStalker {
 public:
  static const std::shared_ptr<ResidualStalker<ResidualVecSize>> Create() { return std::make_shared<ResidualStalker<ResidualVecSize>>(); }

  /// (Friends only access) Toggle status whether stalker object is supervised by evaluation callback. Under supervision, stalker is eglible
  ...
  /// If true, stalker is allowed to obtain residuals for the current optim iteration from ceres cost functor it is attached to. The
  /// internal stalker flag that decides whether stalking is valid for current iter MUST BE CONTROLLED by its supervising
  /// FusionEvaluationCallback.
  ...
  /// Copy calculated ceres residual vector of current optim iteraton to stalker object. This should only ever be called by the ceres-cost
  /// functor this stalker is assigned to.
  void StalkResidualVector(const Eigen::Matrix<double, ResidualVecSize, 1>& residual) {
    ...
    // safety measure to refuse next attempt to pass residuals. Permission to stalk needs to be set externally be supervisor again.
    this->is_jacobian_iter = false;
    this->iter_count++;
  }

  /// Overload that accepts ceres::Jets as template type that occur during an jacobian iter step. Do nothing in this overload, otherwise the
  /// jet type residuals would blow up the whole stalking process. Do not delete this overload, otherwise the jet types would break
  /// compilaition for the stalker that expects double tpyed matrices instead of jets. This should only ever be called by the ceres-cost
  /// functor this stalker is assigned to.
  template <typename T>
 ...
 private:
  ...
  /// stalker is registerd in the FusionEvaluationCallback. On its own, neither the stalker nor the cost functor it is attached to know if
  /// active cost calculation step is a jacobian evaluation step or not.
  bool is_supervised_by_evaluation_callback = false;

  /// Ceres cost evaluation iteration is actually 2 steps. The cost functor is called 2 times in the same iteration. First, the residual
  /// vector calculation for each factor, second the jacobian evaluation. For the second step (jacobian evaluation call), the resisudals are
  /// non - stalkable values and should not be passed by each cost functor to an associated residual stalker. Stalker is notified by its
...
/**
 * @brief Grouped stalker objects that track the calculated residual values, caused by each ceres cost function during optimization. Needs
 * to be attached to an FusionEvaluationCallback and each registered stalker object needed to be attached to an to-be-tracked ceres
 * cost-functor as well.
 *
 */
