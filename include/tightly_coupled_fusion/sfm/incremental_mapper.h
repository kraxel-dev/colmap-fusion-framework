 ...
 * @brief Drived versions of colmaps default IncrementalMapper to:
 ...
 use it in your own script to perform incremental mapping from scratch or have a look at the incremental_pipeline class in
 original colmap lib, that acts as an wrapper to perform the mapping duties you see in the default colmap gui.
...
class IncrementalMapperRerun : public colmap::IncrementalMapper {
 public:
  ...
  /**
   * @brief Derived to swap out DefaultBA with rerun DefaultBA (besides that, exact implementation as parent). Adjust locally
   * connected images and points of a reference image. In addition, refine the provided 3D points. Only images connected to the
   * reference image are optimized. If the provided 3D points are not locally connected to the reference image, their observing
   ...
   */
  colmap::IncrementalMapper::LocalBundleAdjustmentReport AdjustLocalBundle(
  ...
  /**
   * @brief Derived to call custom local BA with rerun logging inside of vanilla iterative scheme. Perform multiple rounds of
...
class IncrementalFusionMapper : public colmap::IncrementalMapper {
 public:
  ...
  /// Derived to swap out DefaultBA with custom FusionGraphBA (besides that, exact implementation as parent). Adjust locally
  /// connected images and points of a reference image. In addition, refine the provided 3D points. Only images connected to the
  /// reference image are optimized.
  colmap::IncrementalMapper::LocalBundleAdjustmentReport AdjustLocalBundle(
      const Options& options,
      const colmap::BundleAdjustmentOptions& ba_options,
      const colmap::IncrementalTriangulator::Options& tri_options,
  ...
  /// Derived to call multiple rounds of derived global bundle adjustment with fusion capabilities.
  void IterativeGlobalRefinement(int max_num_refinements,
                                 double max_refinement_change,
                                 const Options& options,
                                 const colmap::BundleAdjustmentOptions& ba_options,
  ...
  /// Set fusion graph edges (time sorted image node sequence with odometry edges) that will be utilized by ceres optimization.
  /// Required if mapper with odometry fusion capabilities is desired. Provide the unfiltered range of sequential edges that
  /// would span the full colmap model after succ. reconstruction.
