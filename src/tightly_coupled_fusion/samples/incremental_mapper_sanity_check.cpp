 ...
 * @author kraxel
 * @brief Sanity check for the vanilla IncrementalMapper class. Applies manual steps of Mapper (described in: orig colmap repo
 * src/colmap/sfm/incremental_mapper.h) to reconstruct a model from scratch with without fusion capabilities. Order of images will be sorted
 * by ascending time. Very first and 2nd image (by time) in database are forced as initial pair for mapping. Camera intrinsics are fixed.
 * @version 0.1
 * @date 2025-03-24
 *
...
int main(int argc, char** argv) {
  ...
  col_options.AddDefaultOption("time_diff_local_ba",
                               &fusion_ba_options.time_diff_local_ba);  // seconds to pass to allow new round of local BA
  ...
  if (rr_options.is_log_to_rerun) {
    ...
    rrfuse::LogReconstruction(rr_rc->GetRerunRec(),
  ...
  VLOG(1) << "Kickk off a round of global bundle adjustment for initial par!";
  ...
  for (const auto& [stamp, img_id] : img_ids_sorted) {
    ...
    if (stamp - prev_stamp > fusion_ba_options.time_diff_local_ba) {
      VLOG(2) << "More than " << fusion_ba_options.time_diff_local_ba
