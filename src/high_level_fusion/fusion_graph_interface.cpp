...
hifuse::FusionGraphInterface::FusionGraphInterface(const std::shared_ptr<colmap::Reconstruction> reconstruction,
    ...
    : ceres_graph{ceres_graph}, reconstruction{reconstruction}, is_track_residuals{track_residuals} {
...
void hifuse::FusionGraphInterface::AddReprojectionFactors(const colmap::image_t img_id,
  ...
  for (const colmap::Point2D& point2D : img.Points2D()) {
    ...
    reproj_residual_ids_curr_img.push_back(ceres_graph.AddResidualBlock(
        cost_function, new ceres::HuberLoss(1.5), q_cw, t_cw, point3D.xyz.data(), camera_params));  // FIXME loss function from
                                                                                                    // ColmapBAOpts
